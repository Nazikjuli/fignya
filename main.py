...
async def start(message: types.Message):
	...
	if len(result) == 0:
		...
		f'🤘Привет, {message.from_user.mention}, 📝 Цель игры:  ⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀ ⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀                                                		                                              ├─ Кликаем!🎉                                                                                            ├─Выводим💸                                                                                                                         ⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀                                                                                    📢 Новости 👉https://t.me/golclicker  ',
		reply_markup=k.menu)
...
async def reaction(message: types.Message):
	...
	if message.text == '👤 Баланс':
		...
		await message.answer(f'✅Личный кабенет✅                                                                                   ⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀                                          Ваш баланс💰: {fc.toFixed(bal[0], 1)}₽                                                               ⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀                                                                          Реф. ссылка👤 - @GOLCLICKER_BOT')
	elif message.text == '⚽Гол!!':
		q.execute(f'UPDATE users SET balance = balance + 0.005 WHERE user_id IS "{message.chat.id}"')
		connection.commit()
		
		await message.answer('ГОЛ!⚽ вам начислено +0.005₽!')
		...
		await message.answer('-♦️Создатель ботов, Шелбик - @SoftbotPo, всю информацию вы можете найти выше канала!♦️-')
